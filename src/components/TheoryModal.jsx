...
const TheoryModal = ({ openedTask, onClose }) => {
    ...
    const getTheoryData = () => {
        console.log("Mission Name received:", openedTask.mission); // Це покаже в консолі, що саме ми отримали

        // Використовуємо .includes() для всього тексту,
        // додаємо .toUpperCase() для надійності, щоб не було проблем з регістрами
        const missionName = openedTask.mission.toUpperCase();

        if (missionName.includes('МІСІЯ 1')) return mission1Theory;
        if (missionName.includes('МІСІЯ 2')) return mission2Theory;
        if (missionName.includes('МІСІЯ 3')) return mission3Theory;
        if (missionName.includes('МІСІЯ 4')) return mission4Theory;
